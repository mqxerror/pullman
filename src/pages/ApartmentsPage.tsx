...
export default function ApartmentsPage() {
  ...
  const [availableOnly, setAvailableOnly] = useState(() => !searchParams.get('status') || searchParams.get('status') === 'available')
  ...
  const [showFilters, setShowFilters] = useState(() => {
    ...
    return !!(searchParams.get('size') || searchParams.get('budget') || searchParams.get('status'))
  })
  ...
  const filteredApartments = useMemo(() => {
    ...
    // Budget filter
    if (budgetFilter !== 'all') {
      const threshold = Number(budgetFilter) * 1000
      result = result.filter(apt => {
        const price = SUITE_PRICES[apt.unit_number] || 0
        if (budgetFilter === '1000') return price >= 1000000
  ...
  const stats = {
    total: projectConfig.building.totalUnits, // Always show 126
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      <div className="mb-4">
                        ...
                        <p className="text-2xl font-bold text-primary">{formatPriceUSD(SUITE_PRICES[apt.unit_number])}</p>
                      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-bold text-primary">{formatPriceUSD(SUITE_PRICES[apt.unit_number])}</p>
                    </div>
